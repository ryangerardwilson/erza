...
usage() {
  cat <<'EOF'
Push the current repo commit, create the next release tag, wait for the GitHub
release to publish, then upgrade the installed erza app.
...
run_local_tests() {
  ...
  "$test_venv/bin/pip" install --disable-pip-version-check -U pip >/dev/null
  if [[ -s "$REQUIREMENTS_FILE" ]]; then
    "$test_venv/bin/pip" install --disable-pip-version-check -r "$REQUIREMENTS_FILE" >/dev/null
  fi
  python_cmd="$test_venv/bin/python"

  PYTHONPATH="$SRC_DIR" "$python_cmd" -m unittest discover -s "$TEST_DIR"
...
install_requested_release() {
  ...
  for ((attempt = 1; attempt <= INSTALL_ATTEMPTS; attempt += 1)); do
    if bash "$INSTALL_SCRIPT" -v "$version"; then
      return 0
    fi
    rc=$?
...
main() {
  ...
  [[ -f "$VERSION_FILE" ]] || die "_version.py not found at $VERSION_FILE"
  grep -qx '__version__ = "0.0.0"' "$VERSION_FILE" || die "_version.py must remain at 0.0.0 before tagging"
